...
class EdRecSeq2SeqLM(Seq2SeqLM):
  ...
  def __init__(
    ...
    # The backbone returns a dict; we likely want the decoder output for the LM head
    # if both are present, or just use what makes sense.
  ...
  def call_decoder_with_cache(
    ...
    return logits, hidden_states, self_attention_cache, cross_attention_cache
  ...
  def _initialize_cache(self, encoder_token_ids, decoder_token_ids):
    ...
    shape = [batch_size, num_layers, 2, decoder_max_length, num_heads, head_dim]
