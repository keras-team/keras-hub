...
class StableLMCausalLM(CausalLM):
    ...
    def generate_step(self, inputs, stop_token_ids=None):
        ...
        # Because StableLM uses the same token for padding and EOS, the generic
        # sampler will immediately terminate if it sees stop_tokens in the
        # padding area.We temporarily replace padding tokens with a safe dummy
        # token.
        if stop_token_ids is not None:
            dummy_token_id = 0
            while dummy_token_id in stop_token_ids:
                dummy_token_id += 1
            safe_pad = ops.cast(
                ops.ones_like(token_ids) * dummy_token_id, token_ids.dtype
            )
            token_ids = ops.where(padding_mask, token_ids, safe_pad)
        ...
        if stop_token_ids is not None:
            ...
            # Restore the original pad/eos tokens in the unused tail of the
            # sequence
            token_ids = ops.where(
                new_padding_mask, token_ids, inputs["token_ids"]
            )
