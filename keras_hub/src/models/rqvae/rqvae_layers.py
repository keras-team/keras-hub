...
class VectorQuantizerEMA(layers.Layer):
  ...
  def call(self, inputs, training=False):
    ...
    if training:
      # EMA Update
      # cluster_size = ema_cluster_size * decay + (1-decay) * sum(encoding)
      ...
      updated_ema_w = (
          self.ema_w * self.decay + (1.0 - self.decay) * total_assignment_sums
      )
      ...
      # Quantization loss
      quantization_loss = ops.mean((flattened_inputs - quantized_flat) ** 2)
...
class ResidualVectorQuantizer(layers.Layer):
  ...
  def call(self, inputs, training=False):
    ...
    for quantizer in self.quantizers:
      ...
      current_quantized, current_encoding, usage_ratio, quantization_loss = (
          quantizer(residual, training=training)
      )
      total_quantization_loss = total_quantization_loss + quantization_loss
