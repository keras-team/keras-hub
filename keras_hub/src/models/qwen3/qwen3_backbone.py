...
class Qwen3Backbone(Backbone):
    ...
    def get_layout_map(
        ...
        with distribution.scope():
           qwen3_model = keras_hub.models.Qwen3Backbone.from_preset()
        ```

        To see how the layout map was applied, load the model then run
        (for one decoder block):
        ```
        embedding_layer = qwen3_model.backbone.get_layer("token_embedding")
        decoder_block_1 = qwen3_model.backbone.get_layer('transformer_layer_0')
