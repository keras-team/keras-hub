...
class NoiseScheduler:
    ...
    def __init__(
        ...
        self.alphas_cumprod = keras.ops.cumprod(self.alphas)
    ...
    def _get_variance(self, timestep, predicted_variance=None):
        ...
        if self.variance_type == "fixed_small":
            ...
            variance = keras.ops.log(self.betas[timestep])
    ...
    def step(
        ...
        if model_output.shape[1] == sample.shape[1] * 2 and self.variance_type in [
            ...
            model_output, predicted_variance = keras.ops.split(
        ...
        if self.clip_sample:
            pred_original_sample = keras.ops.clip(pred_original_sample, -1, 1)
    ...
    def add_noise(
        ...
        sqrt_alpha_prod = keras.ops.take(self.alphas_cumprod, timesteps) ** 0.5
        sqrt_one_minus_alpha_prod = (
            1 - keras.ops.take(self.alphas_cumprod, timesteps)
        ) ** 0.5

        for _ in range(3):
            sqrt_alpha_prod = keras.ops.expand_dims(sqrt_alpha_prod, axis=-1)
            sqrt_one_minus_alpha_prod = keras.ops.expand_dims(
