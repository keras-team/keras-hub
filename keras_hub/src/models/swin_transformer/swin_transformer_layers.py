...
def window_partition(x, window_size):
    ...
    if len(shape) != 4:
        raise ValueError(
            f"Expected input tensor to have 4 dimensions, got {len(shape)}"
        )
    ...
    # Reshape to windows
    x = ops.reshape(
        x, (B, num_windows_h, window_size, num_windows_w, window_size, C)
    )
...
def window_reverse(windows, window_size, height, width, channels):
    ...
    # Reshape windows to [B, num_windows_h, num_windows_w, window_size,
    # window_size, C]
    x = ops.reshape(
        windows,
        (
...
class DropPath(layers.Layer):
    ...
    This is an implementation of the paper "Deep Networks with Stochastic
    Depth", which randomly drops entire layers for regularization.
    ...
    def call(self, x, training=None):
        ...
        random_tensor = keep_prob + keras.random.uniform(
            mask_shape, seed=self.seed_generator
        )
...
class WindowAttention(keras.layers.Layer):
    ...
    Args:
        ...
        head_dim (int): Number of channels per head (dim // num_heads if not
            set)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.relative_position_index = get_relative_position_index(
            win_h=self.win_h, win_w=self.win_w
        )
...
class SwinTransformerBlock(keras.layers.Layer):
    ...
    Args:
        ...
        norm_layer (keras.layers.Layer, optional): Normalization layer.
            Default: keras.layers.LayerNormalization(epsilon=1e-5)
    """
    ...
    def call(self, x):
        ...
        x_windows, (H_pad, W_pad) = window_partition(
            x=shifted_x, window_size=self.window_size
        )
        ...
        shifted_x = window_reverse(
            attn_windows, self.window_size, H_pad, W_pad, C
        )
...
class PatchMerging(layers.Layer):
    ...
    This layer performs downsampling by concatenating patches and using a
    linear layer.
...
class SwinTransformerStage(layers.Layer):
    ...
    A stage consists of multiple Swin Transformer blocks with the same
    resolution, and an optional patch merging layer at the beginning.
    ...
    def call(self, x):
        ...
        Returns:
            Output feature with shape [B, H/2*W/2, 2*C] if downsample is
            applied, otherwise [B, H*W, C].
        """
    ...
    def from_config(cls, config):
        config["downsample"] = (
            keras.utils.deserialize_keras_object(config["downsample"])
