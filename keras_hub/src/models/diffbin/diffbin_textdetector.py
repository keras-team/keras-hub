...
class DiffBinTextDetector(keras.Model):
    ...
    Args:
        ...
        preprocessor: An `ImageTextDetectorPreprocessor` instance.
            Defaults to `None`, in which case the default preprocessor will be
            used.
        **kwargs: Additional keyword arguments for the `keras.Model` class.

    Attributes:
        backbone: The `DiffBinBackbone` instance used for feature extraction.
        preprocessor: The `ImageTextDetectorPreprocessor` instance used for
            preprocessing input images.
    """

    backbone_cls = DiffBinBackbone
    preprocessor_cls = ImageClassifierPreprocessor
