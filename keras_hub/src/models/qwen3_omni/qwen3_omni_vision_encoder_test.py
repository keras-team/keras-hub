...
class Qwen3OmniVisionEncoderTest(TestCase):
    ...
    def test_patch_embed_realistic_shape(self):
        patch_embed = Qwen3OmniVisionPatchEmbed(
            patch_size=16,
            temporal_patch_size=2,
            in_channels=3,
        ...
        pixel_values = np.random.rand(4, 2, 16, 16, 3).astype("float32")
        output = patch_embed(pixel_values)
        self.assertEqual(output.shape, (4, 32))
    ...
    def test_encoder_output_shape(self):
        ...
        # 1 image, 1 temporal, 2x2 spatial grid -> 4 tokens.
        self.assertEqual(output["last_hidden_state"].shape, (1, 4, 32))
        self.assertEqual(output["pooler_output"].shape[-1], 16)

    def test_encoder_realistic_patch_shape(self):
        encoder = Qwen3OmniVisionEncoder(
            depth=1,
            hidden_size=32,
            num_heads=4,
        ...
        pixel_values = np.random.rand(16, 2, 16, 16, 3).astype("float32")
        # 1 image, 1 temporal, 4x4 spatial grid -> 16 tokens total.
        grid_thw = np.array([[1, 4, 4]], dtype="int32")

        output = encoder({"pixel_values": pixel_values, "grid_thw": grid_thw})

        self.assertEqual(output["last_hidden_state"].shape, (1, 16, 32))
        # Pooled tokens = 16 / (spatial_merge_size ** 2) = 4.
        self.assertEqual(output["pooler_output"].shape, (1, 4, 16))
