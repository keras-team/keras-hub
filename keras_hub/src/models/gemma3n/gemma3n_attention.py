...
class Gemma3nAudioRelativePositionEmbedding(keras.layers.Layer):
    ...
    Args:
        ...
        num_attention_context_left: int. The number of steps to attend to in
            the past, including the current step.
        num_attention_context_right: int. The number of steps to attend to in
            the future.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.head_dim = self.channels // self.num_attention_heads
        self.max_backward = max(0, num_attention_context_left - 1)
        self.max_forward = num_attention_context_right
        ...
        self.inv_timescales = ops.expand_dims(
            ops.expand_dims(
                ops.convert_to_tensor(inv_timescales, dtype="float32"), 0
            ),
            0,
        )
    ...
    def _get_timing_signal_1d_pos(self, position, dtype):
        """Generate sinusoidal timing signals for relative positions."""
        position = ops.cast(ops.expand_dims(position, axis=-1), "float32")
        pos_shape = ops.shape(position)
        inv_shape = ops.shape(self.inv_timescales)
        target_shape = (pos_shape[0], pos_shape[1], inv_shape[2])
        position = ops.broadcast_to(position, target_shape)
        inv_timescales = ops.broadcast_to(self.inv_timescales, target_shape)
        scaled_time = position * inv_timescales
        timing_signal = ops.concatenate(
            [ops.sin(scaled_time), ops.cos(scaled_time)], axis=-1
        )
        return ops.cast(timing_signal, dtype)
    ...
    def _relative_shift(
        ...
        """Shift relative position bias to align with attention logits."""
        msp1_val = max_span_plus_1
        kcs_val = key_context_size
        if not isinstance(msp1_val, int) and hasattr(msp1_val, "shape"):
            msp1_val = ops.shape(msp1_val)[-1]
        if not isinstance(kcs_val, int) and hasattr(kcs_val, "shape"):
            kcs_val = ops.shape(kcs_val)[-1]
        pad_amount_last_dim = (kcs_val + 1) - msp1_val
        padding_tuple = [[0, 0]] * (
            len(ops.shape(term_bd_before_shift)) - 1
        ) + [[0, pad_amount_last_dim]]
        term_bd_padded = ops.pad(term_bd_before_shift, padding_tuple)
        shape_padded = ops.shape(term_bd_padded)
        B = shape_padded[0]
        H = shape_padded[1]
        U = shape_padded[2]
        W = shape_padded[3]
    ...
    def call(self, queries, keys):
        ...
        max_span_plus_1 = ops.shape(pos_indices)[1]
        ...
        sin_emb = ops.squeeze(
            ops.reshape(
                projected_sin_emb,
                (1, max_span_plus_1, self.num_attention_heads, self.head_dim),
            ),
            axis=0,
        )
        ...
        q_permuted = ops.transpose(queries, (0, 3, 1, 2, 4))
        s_permuted = ops.transpose(sin_emb, (1, 2, 0))
        term_bd_unshifed = ops.einsum(
...
class Gemma3nTextAttention(keras.layers.Layer):
    ...
    def _mask_sliding_window(
        ...
        """Apply sliding window mask to limit attention to local context."""
        ...
        if keras.config.backend() == "tensorflow":
            ...
            band_size = ops.minimum(key_len, self.sliding_window - 1)
            ...
            ) * ops.tril(all_ones, self.sliding_window - 1)
    ...
    def _compute_attention(
        ...
        """Compute scaled dot-product attention with
        optional masking and dropout."""
        scaling = 1.0
        key_states = self.repeat_kv(key)
        value_states = self.repeat_kv(value)
        attn_weights = (
            ops.matmul(query, ops.transpose(key_states, (0, 1, 3, 2))) * scaling
        )
    ...
    def call(
        ...
        if self.is_kv_shared_layer:
            ...
            key_update = ops.transpose(key_update, (0, 2, 1, 3))
            value_update = self.v_proj(hidden_states)
            value_update = ops.reshape(
                value_update,
                input_shape + (self.num_key_value_heads, self.head_dim),
            )
            value_update = self.v_norm(value_update)
            value_update = ops.transpose(value_update, (0, 2, 1, 3))
...
class Gemma3nAudioAttention(keras.layers.Layer):
    ...
    Args:
        hidden_size: int. The size of the hidden state.
        num_attention_heads: int. The number of attention heads.
        attention_chunk_size: int. The size of each processing chunk.
        num_attention_context_right: int. The number of steps to attend to in
            the future.
        num_attention_context_left: int. The number of steps to attend to in
            the past, including the current step.
        attention_logit_cap: float. The soft cap value to apply to the
            attention logits.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_future_horizon = num_attention_context_right
        self.max_past_horizon = max(0, num_attention_context_left - 1)
        self.context_size = (
        ...
        lower_causal_mask = np.tril(
            np.ones((self.context_size, self.attention_chunk_size), dtype=bool),
            k=0,
        ).T
        upper_causal_mask = np.tril(
            np.ones((self.attention_chunk_size, self.context_size), dtype=bool),
            k=self.max_past_horizon + self.max_future_horizon,
        )
        local_causal_valid_mask = np.ones(
            (self.attention_chunk_size, self.context_size), dtype=bool
        )
        local_causal_valid_mask = (
            local_causal_valid_mask * lower_causal_mask * upper_causal_mask
        )
        self.local_causal_valid_mask = ops.convert_to_tensor(
            local_causal_valid_mask
        )
        self.softcap = ops.convert_to_tensor(
            self.attention_logit_cap, dtype="float32"
        )
    ...
    def _pad_dim1(self, x, pad_left, pad_right):
        """Pad the second dimension (time axis) with zeros."""
        paddings = [[0, 0], [pad_left, pad_right]] + [
            [0, 0] for _ in range(len(ops.shape(x)) - 2)
        ]
        return ops.pad(x, paddings)

    def _convert_to_block(self, hidden_states):
        """Reshape sequence into fixed-size blocks for chunked attention."""
        b, t = ops.shape(hidden_states)[:2]
        tail_shape_list = list(hidden_states.shape[2:])
        num_blocks = (
            t + self.attention_chunk_size - 1
        ) // self.attention_chunk_size
        padding_len = num_blocks * self.attention_chunk_size - t
        ...
        return ops.reshape(hidden_states, permute_dims)

    def _extract_block_context(self, hidden_states):
        """Extract overlapping context windows for each attention block."""
        _, t = ops.shape(hidden_states)[:2]
        num_frames = (
            t + self.attention_chunk_size - 1
        ) // self.attention_chunk_size
        pad_left = self.max_past_horizon
        pad_right = self.max_future_horizon + self.attention_chunk_size - 1
        hidden_states = self._pad_dim1(hidden_states, pad_left, pad_right)
        frame_len = self.context_size
        frame_step = self.attention_chunk_size

        start_indices = ops.arange(0, num_frames) * frame_step
        frame_offsets = ops.arange(0, frame_len)
        indices = ops.expand_dims(start_indices, axis=1) + ops.expand_dims(
            frame_offsets, axis=0
        )
        return ops.take(hidden_states, indices, axis=1)
    ...
    def call(self, hidden_states, mask):
        ...
        original_valid_mask = ops.logical_not(mask)
        extracted_valid_mask_blocks = self._extract_block_context(
            original_valid_mask
        )
        mask_block_shape = ops.shape(extracted_valid_mask_blocks)
        if len(mask_block_shape) > 3:
            ...
            if axes_to_squeeze:
                extracted_valid_mask_blocks = ops.squeeze(
        ...
        if (
            ...
            extracted_valid_mask_blocks = ops.reshape(
        ...
        final_condition_for_where = ops.logical_and(
            condition_from_input_validity,
            ops.cast(condition_from_causality, "bool"),
        )
        logits = self.relative_position_embedding(query_blocks, key_blocks)
        softcap = ops.cast(self.softcap, dtype=logits.dtype)
        ...
        min_val = ops.convert_to_tensor(min_val, dtype=compute_dtype)
        logits = ops.where(final_condition_for_where, logits, min_val)
        probabilities = ops.softmax(ops.cast(logits, "float32"), axis=-1)
        probabilities = ops.cast(probabilities, value_blocks.dtype)
        ...
        context_vectors = ops.reshape(
            ...
            (
                batch_size,
                num_query_blocks * self.attention_chunk_size,
                self.num_attention_heads,
                self.head_dim,
            ),
        )
