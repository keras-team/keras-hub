...
class T5Gemma2BackboneTest(TestCase):
    ...
    def test_layout_map_mesh_shapes_skips_when_query_heads_indivisible(self):
        ...
        # rule happens not to fire): T5GEMMA2_270M_DIMS' real-preset ratio
        # has num_attention_heads=4, so a model-axis size of 3 -- which does
        # not divide 4 -- must trigger the skip, proving the guard actually
        ...
        # skip idiom used inside test_layout_map_mesh_shapes directly (that
        # method is parameterized via @parameterized.named_parameters, which
        # replaces the base method with per-case variants, so it can't be
        ...
        model_axis_size = mesh_shape[-1]
        num_query_heads = T5GEMMA2_270M_DIMS["num_attention_heads"]
        self.assertNotEqual(num_query_heads % model_axis_size, 0)

        def _build_with_skip_guard():
            if num_query_heads % model_axis_size != 0:
                self.skipTest(
                    f"num_attention_heads={num_query_heads} not divisible "
                    f"by model-axis={model_axis_size}: inherent "
                    "tensor-parallelism limit, not a bug"
                )
            axis_names = ("batch", "model")
            device_mesh = keras.distribution.DeviceMesh(
            ...
            return T5Gemma2Backbone.get_layout_map(device_mesh)
