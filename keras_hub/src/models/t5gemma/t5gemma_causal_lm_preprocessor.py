...
class T5GemmaCausalLMPreprocessor(CausalLMPreprocessor):
    """T5Gemma Causal LM preprocessor.

    This preprocessing layer is meant for use with
    `keras_hub.models.T5GemmaCausalLM`. By default, it will take in batches of
    strings, and return outputs in a `(x, y, sample_weight)` format, where the
    `y` label is the next token id in the `x` sequence.

    For use with generation, the layer also exposes two methods
    `generate_preprocess()` and `generate_postprocess()`. When this preprocessor
    is attached to a `keras_hub.models.T5GemmaCausalLM` instance, these methods
    ...
    preprocessor = keras_hub.models.T5GemmaCausalLMPreprocessor.from_preset(
