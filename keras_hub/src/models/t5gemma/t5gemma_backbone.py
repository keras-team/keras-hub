...
class T5GemmaBackbone(Backbone):
    ...
    def get_layout_map(
        ...
        # tie_word_embeddings defaults to True, but is a real constructor
        # param that convert_t5gemma.py propagates from live HF configs, and
        # untied real presets exist (see t5gemma_seq_2_seq_lm_test.py's
        # tie_word_embeddings=False config) -- so a separate
        # decoder_token_embedding/reverse_embeddings output-projection
        # tensor can exist and must not be silently left mis-sharded. The
        # tensor is (hidden_dim, vocabulary_size); vocab-parallel output
        # projection wants vocab sharded on the model axis, consistent with
        # decoder_token_embedding/embeddings (vocab, hidden) =
        # (model_dim, data_dim) above.
        layout_map["decoder_token_embedding/reverse_embeddings"] = (
            data_dim,
            model_dim,
        )
        # Query/key/value kernels are (hidden_dim, num_heads, head_dim) --
        # the contracting hidden dim goes on the data axis and heads go on
        # the model axis (Megatron column-parallel) for query, matching
        # attention_output's own (heads, head_dim, hidden) -> (model, None,
        # data) row-parallel convention below. Key/value kernels are sized
        # by num_key_value_heads (GQA), which is independent of and
        # typically much smaller than num_query_heads -- sharding that
        # small heads axis on the model-parallel dim would raise an
        # IndivisibleError whenever the model mesh dimension doesn't divide
        # it, so the heads axis is left fully replicated on model while the
        # hidden (contracting) dim is sharded on data for memory savings,
        # mirroring the Gemma-family convention. The broad "attention"
