...
class InceptionBackbone(FeaturePyramidBackbone):
    ...
    Args:
        ...
        aux_classifiers: boolean. Whether to include auxiliary classifiers or 
            not. Note: In backbone mode, these are typically not used.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, (filters, stride) in (
            enumerate(zip(initial_filters, initial_strides))):
            if stride > 1:
            ...
            x = layers.ReLU(dtype=dtype, name=f"conv{i+1}_relu")(x)
        ...
        for i, config in enumerate(inception_config):
            ...
            if aux_classifiers and (
                ...
                # Auxiliary classifier output not used in backbone mode
...
def apply_inception_module(
    ...
    Args:
        ...
        filters_3x3_reduce: int. The number of filters in the 3x3 reduce 
            convolution.
        filters_3x3: int. The number of filters in the 3x3 convolution.
        filters_5x5_reduce: int. The number of filters in the 5x5 reduce 
            convolution.
    ...
    branch1 = layers.ReLU(dtype=dtype, name=f"{name}_1x1_relu")(branch1)
    ...
    branch2 = layers.ReLU(dtype=dtype, name=f"{name}_3x3_reduce_relu")(branch2)
    branch2 = layers.Conv2D(
        filters_3x3,
        3,
        padding="same",
    ...
    branch2 = layers.ReLU(dtype=dtype, name=f"{name}_3x3_relu")(branch2)
    ...
    branch3 = layers.ReLU(dtype=dtype, name=f"{name}_5x5_reduce_relu")(branch3)
    branch3 = layers.Conv2D(
        filters_5x5,
        5,
        padding="same",
    ...
    branch3 = layers.ReLU(dtype=dtype, name=f"{name}_5x5_relu")(branch3)
    ...
    branch4 = layers.ReLU(dtype=dtype, name=f"{name}_pool_proj_relu")(branch4)
...
def apply_auxiliary_classifier(
    ...
    # These would normally include flatten, dense (1024), dropout, and final 
    # dense layer
