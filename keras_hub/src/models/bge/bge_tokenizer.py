...
@keras_hub_export(
    [
        "keras_hub.tokenizers.BgeTokenizer",
        "keras_hub.models.BgeTokenizer",
    ]
)
class BgeTokenizer(WordPieceTokenizer):
    """A BGE tokenizer using WordPiece subword segmentation.

    This tokenizer class will tokenize raw strings into integer sequences and
    is based on `keras_hub.tokenizers.WordPieceTokenizer`. Unlike the
    underlying tokenizer, it will check for all special tokens needed by BGE
    models and provides a `from_preset()` method to automatically download a
    matching vocabulary for a BGE preset.

    BGE uses the standard BERT uncased WordPiece vocabulary (30,522 tokens)
    with case-insensitive (lowercase) normalization. The tokenizer prepends
    `[CLS]` (id=101) and appends `[SEP]` (id=102) to every sequence.
    ...
    Args:
        ...
        lowercase: If `True`, the input text will be first lowercased before
            tokenization. BGE presets set this to `True`.
    ...
    ```python
    # Unbatched input.
    tokenizer = keras_hub.models.BgeTokenizer.from_preset(
        "bge_small_en_v1.5",
    )
