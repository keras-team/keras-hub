...
@keras_hub_export("keras_hub.models.Gemma4UnifiedCausalLMPreprocessor")
class Gemma4UnifiedCausalLMPreprocessor(CausalLMPreprocessor):
    """Gemma4 Unified Causal LM preprocessor.

    This preprocessing layer is meant for use with
    `keras_hub.models.Gemma4UnifiedCausalLM`. It can be configured in three
    ...
    Args:
        ...
        image_converter: A `keras_hub.layers.Gemma4UnifiedImageConverter`
            instance. Defaults to `None`.
        audio_converter: A `keras_hub.layers.Gemma4UnifiedAudioConverter`
            instance. Defaults to `None`.
        video_converter: A `keras_hub.layers.Gemma4UnifiedVideoConverter`
            instance. Defaults to `None`.
    ...
    image_converter_cls = Gemma4UnifiedImageConverter
    audio_converter_cls = Gemma4UnifiedAudioConverter
    video_converter_cls = Gemma4UnifiedVideoConverter
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Mel-spectrogram frequency bins for dummy audio tensors.
        self.audio_input_feat_size = audio_input_feat_size

        # Text-only if no converters are passed.
    ...
    def build(self, input_shape):
        # Defer packer creation to build() for saved model compatibility.
    ...
    def _get_vision_indices(self, vision_mask, max_tokens=None):
        ...
        # Pad to max_tokens (fixed) or max actual count (dynamic).
    ...
    def _format_output(
        ...
        # Sequential position IDs for all tokens (including audio).
    ...
    def _preprocess_audio(self, audio, batched):
        ...
        # Move CUDA tensors to CPU so subsequent TF ops can accept them.
        if keras.config.backend() == "torch":
    ...
    def generate_preprocess(
        ...
        if self.audio_converter is not None:
            # Replace single <|audio|> placeholder with expanded sequence.
            prompts = tf.strings.regex_replace(
                prompts,
                re.escape(self.audio_placeholder),
                replacement,
        ...
        if audio is not None and self.audio_converter is not None:
            # audio_mel and audio_mel_mask computed earlier.
            ...
            # Remove unused placeholders to keep sequence compact.
            keep_mask = ~is_unused
            batch_size = tf.shape(token_ids)[0]
            seq_len = tf.shape(token_ids)[1]
    ...
    def generate_postprocess(
        ...
        # Strip audio soft tokens even when tokenizer lacks has_audio_tokens.
        # Resolve real vocab IDs via token_to_id() as fallback.
        if self.audio_converter is not None:
            for tok_id in [
        ...
        # Strip residual audio token strings that survive as byte-fallback
        # pieces (making ID-based stripping ineffective).
        if self.audio_converter is not None:
            for tok_str in [
                self.audio_placeholder,  # "<|audio|>"
                self.start_of_audio_token,  # "<|audio>"
