...
@keras_hub_export("keras_hub.models.Gemma4UnifiedBackbone")
class Gemma4UnifiedBackbone(Backbone):
    """Gemma4 Unified (encoder-free) backbone.

    This backbone implements the Gemma4 Unified model architecture. Unlike
    the standard Gemma4 backbone that uses a SigLIP vision encoder, this
    unified model uses an encoder-free vision embedder
    (`Gemma4UnifiedVisionEmbedder`) that directly projects pre-merged image
    patches into the language model's hidden space.
    ...
    * **Audio encoder** — an optional Universal Speech Model (USM) conformer
    ...
    `keras_hub.models.Gemma4UnifiedCausalLM`.

    The default constructor gives a fully customised, randomly initialised
    Gemma4 Unified model. To load preset weights use the `from_preset`
    constructor.
    ...
    Args:
        ...
        image_size: int. The spatial resolution of images (height = width).
            ...
            are handled by `keras_hub.layers.Gemma4UnifiedImageConverter`
            before data reaches the backbone. The `vision_encoder` has its
            own `image_size` parameter that controls position embedding
            sizes.
        ...
        vision_encoder: `keras_hub.models.Gemma4UnifiedVisionEmbedder` or
            `None`. When `None` the model processes no images.
    ...
    ```python
    # Load a pretrained Gemma4 Unified backbone.
    model = keras_hub.models.Gemma4UnifiedBackbone.from_preset(
        "gemma4_unified_instruct_12b"
    )

    # Randomly initialized Gemma4 Unified backbone with custom config.
    vision_embedder = keras_hub.models.Gemma4UnifiedVisionEmbedder(
        hidden_dim=3840,
        model_patch_size=48,
        mm_posemb_size=24,
        num_soft_tokens=280,
    )
    model = keras_hub.models.Gemma4UnifiedBackbone(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Per-layer token-conditioned gating input.
        ...
        # Build transformer layers; precompute KV-sharing indices.
        ...
        if num_kv_shared_layers > 0:
            if _first_kv_shared == 0:
                ...
                # Map shared layer → its KV source.
        ...
        for i in range(num_layers):
            # Global if last in each `sliding_window_pattern` group.
            if layer_types is not None:
        ...
        # Vision inputs (pixel values + position ids).
        if vision_encoder is not None:
        ...
        # Interleave image embeddings, pre-scaled by 1/sqrt(hidden_dim).
        if vision_encoder is not None:
        ...
        # Interleave audio embeddings (same pre-scaling).
        if audio_encoder is not None:
        ...
        # Per-layer token embeddings (vision/audio positions masked to 0).
        if hidden_size_per_layer_input > 0:
        ...
        # Global scale by sqrt(hidden_dim).
        x = x * ops.cast(ops.sqrt(hidden_dim), x.dtype)

        # Per-layer model projection (post global-scale).
        if hidden_size_per_layer_input > 0:
        ...
        for i, transformer_layer in enumerate(self.transformer_layers):
            if per_layer_proj_flat is not None:
                # per_layer_input = (norm(proj_i) + emb_i) * 2^-0.5
