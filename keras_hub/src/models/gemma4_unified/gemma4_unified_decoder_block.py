...
class Gemma4UnifiedTextDecoderBlock(keras.layers.Layer):
    """Transformer decoder layer for Gemma4 Unified (encoder-free) model.

    Implements the text decoder block used by `Gemma4UnifiedBackbone`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # FFW gated GELU. actual_intermediate_dim may be 2x for KV-shared
        # layers when use_double_wide_mlp=True.
        ...
        # MoE blocks (26b-a4b): dense MLP runs in parallel with sparse MoE,
        # summed BEFORE post-FFW norm.
        if enable_moe_block:
            ...
            # Separate dense-path post-FFW norm for MoE blocks.
        ...
        # Per-layer input gate (E4B): gated residual conditioned on per-token embedding.  # noqa: E501
    ...
    def build(self, input_shape):
        ...
        # Text decoder layers have a non-trainable layer_scalar (init 1.0).
    ...
    def _compute_attention_mask(
        ...
        if self.use_bidirectional_attention:
            # Bidirectional attention: return None when no padding.
        ...
        # Restrict causal mask to sliding window BEFORE OR-ing vision mask.
        # HF ordering: (causal AND sliding) OR vision_bidirec.
        ...
        # Vision bidirectional attention for local (sliding-window) layers only.
        if (
        ...
        # Blockwise masking for sliding-window layers: same-block tokens
        # attend bidirectionally (HF blockwise_overlay).
        if (
    ...
    def call(
        ...
        if self.enable_moe_block:
            # Dense + MoE parallel paths (26b-a4b); uses direct matmul.
            ...
            # MoE path: router on raw x; experts on normed x.
            ...
            # Batched GEMM across all experts; sparse combine via dispatch_weights.  # noqa: E501
            ...
            # Standard dense FFW path (direct matmul).
            normalized_x = self.pre_ffw_norm(x)
            x1 = ops.matmul(normalized_x, self.gating_ffw.kernel)
            x2 = ops.matmul(normalized_x, self.gating_ffw_2.kernel)
        ...
        # Per-layer input gate (E4B): gated residual, applied after FFW
        # and before layer_scalar.
