...
def convert_weights(backbone, loader, transformers_config):
    ...
    for i in range(backbone.num_layers):
        ...
        try:
            ...
            gate_up_blocks = loader.get_tensor(f"model.layers.{i}.mlp.experts.gate_up_proj_blocks")
            gate_up_scales = loader.get_tensor(f"model.layers.{i}.mlp.experts.gate_up_proj_scales")
            gate_up_bias = loader.get_tensor(f"model.layers.{i}.mlp.experts.gate_up_proj_bias")

            down_blocks = loader.get_tensor(f"model.layers.{i}.mlp.experts.down_proj_blocks")
            down_scales = loader.get_tensor(f"model.layers.{i}.mlp.experts.down_proj_scales")
            down_bias = loader.get_tensor(f"model.layers.{i}.mlp.experts.down_proj_bias")
            ...
            gate_up_transposed = np.transpose(gate_up_dequantized, (0, 2, 1))  # [32, 1440, 5760]
            # Expand the hidden dimension by repeating each element twice
            gate_up_expanded = np.repeat(gate_up_transposed, 2, axis=1)  # [32, 2880, 5760]
            ...
            down_expanded = np.repeat(down_dequantized, 2, axis=2)  # [32, 2880, 2880]
            down_transposed = np.transpose(down_expanded, (0, 2, 1))  # [32, 2880, 2880]
            ...
            print(f"Successfully loaded dequantized MoE expert weights for layer {i}")

        except KeyError as e:
            print(f"Warning: Could not load MoE expert weights for layer {i}: {e}")
            print(f"Available keys: {[k for k in loader.safetensor_config['weight_map'].keys() if f'layers.{i}.mlp' in k]}")
    ...
    print(f"Token embedding parameters: {backbone.token_embedding.count_params():,}")
    print(f"Output projection parameters: {backbone.token_embedding.reverse_embeddings.shape[0] * backbone.token_embedding.reverse_embeddings.shape[1]:,}")
    print(f"Final layer norm parameters: {backbone.layer_norm.count_params():,}")
