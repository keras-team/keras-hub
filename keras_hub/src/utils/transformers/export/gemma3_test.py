...
class TestGemma3Export(TestCase):
    ...
    def test_exported_files_completeness(self):
        ...
        for file_name in optional_files:
            ...
            if not os.path.exists(file_path):
                print(f"Optional file {file_name} not generated (requires transformers library)")

        # Verify generation_config.json content
        with open(os.path.join(export_path, "generation_config.json"), "r") as f:
        ...
        with open(os.path.join(export_path, "special_tokens_map.json"), "r") as f:
    ...
    def test_rope_parameters_structure(self):
        """Test that rope_parameters are correctly structured for HuggingFace."""
        import json
        ...
        # Full attention should use 1M (1000000.0), sliding should use 10K (10000.0)
        self.assertEqual(rope_params["full_attention"]["rope_theta"], 1000000.0)
        self.assertEqual(rope_params["sliding_attention"]["rope_theta"], 10000.0)
