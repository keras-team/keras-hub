...
def convert_weights(backbone, loader, transformers_config):
    ...
    for i in range(backbone.num_layers):
        ...
        loader.port_weight(
            ...
            hf_weight_key=f"{hf_prefix}.attention.output.LayerNorm.{ln_beta}",
        )
        loader.port_weight(
            keras_variable=block._self_attention_layer_norm.gamma,
            hf_weight_key=f"{hf_prefix}.attention.output.LayerNorm.{ln_gamma}",
        )
        ...
        loader.port_weight(
            ...
            hf_weight_key=f"{hf_prefix}.output.LayerNorm.{ln_beta}",
        )
        loader.port_weight(
            keras_variable=block._feedforward_layer_norm.gamma,
            hf_weight_key=f"{hf_prefix}.output.LayerNorm.{ln_gamma}",
        )


def convert_tokenizer(cls, preset, **kwargs):
    return cls(
        proto=get_file(preset, "sentencepiece.bpe.model"),
        **kwargs,
    )
...
def load_task_config(preset, transformers_config):
    ...
    Returns kwargs suitable for ``XLMRobertaTextEmbedder``.
    """
    kwargs = {}
...
def load_preprocessor_config(preset, transformers_config):
    ...
    For example, BAAI/bge-m3 specifies 8192 here.
    """
