...
class CausalLMExporterConfig(KerasHubExporterConfig):
    ...
    def __init__(self, model):
        ...
        if hasattr(model, 'backbone') and hasattr(model.backbone, 'vision_encoder') and model.backbone.vision_encoder is not None:
            self.EXPECTED_INPUTS = ["token_ids", "padding_mask", "images", "vision_mask", "vision_indices"]
        # Check for PaliGemma-style multimodal (has image_encoder or vit attributes)
        elif self._is_paligemma_style_multimodal(model):
            self.EXPECTED_INPUTS = ["token_ids", "padding_mask", "images", "response_mask"]
    ...
    def _is_paligemma_style_multimodal(self, model):
        ...
        if hasattr(model, 'backbone'):
            ...
            if hasattr(backbone, 'image_size') and (
                hasattr(backbone, 'vit_num_layers') or 
                hasattr(backbone, 'vit_patch_size')
            ):
                return True
        return False
    ...
    def get_input_signature(self, sequence_length=None):
        ...
        Args:
            sequence_length: `int`, `None`, or `dict`. Optional sequence length. 
                ...
                should contain 'sequence_length' and 'image_size' for multimodal models.
        ...
        if hasattr(self.model.backbone, 'vision_encoder') and self.model.backbone.vision_encoder is not None:
            ...
            signature.update({
                ...
                "vision_mask": keras.layers.InputSpec(
                    dtype="int32",  # Use int32 instead of bool for TFLite compatibility
                    shape=(None, None)
                ),
        ...
        # Check if Parseq-style vision
            ...
            if hasattr(self.model.backbone, 'image_encoder') and hasattr(self.model.backbone.image_encoder, 'image_shape'):
...
class AudioToTextExporterConfig(KerasHubExporterConfig):
    ...
    def get_input_signature(self, sequence_length=None, audio_length=None):
        ...
        Args:
            sequence_length: `int` or `None`. Optional text sequence length. If None,
                exports with dynamic shape for flexibility.
            audio_length: `int` or `None`. Optional audio sequence length. If None,
                exports with dynamic shape for flexibility.
...
class ObjectDetectorExporterConfig(KerasHubExporterConfig):
    ...
    def get_input_signature(self, image_size=None):
        ...
        Returns:
            `dict`. Dictionary mapping input names to their specifications
        """
        # Object detectors use dynamic image shapes to support variable input sizes
        # The preprocessor image_size is used for training but export allows any size
        dtype = _infer_image_dtype(self.model)
    ...
    def get_input_signature(self, image_size=None):
        ...
        return {
            ...
            "masks": keras.layers.InputSpec(
                dtype="float32",
                shape=(None, 1, *mask_size, 1)  # Fixed: 1 mask at correct resolution
            ),
        }
...
class TextToImageExporterConfig(KerasHubExporterConfig):
    ...
    def get_input_signature(self, sequence_length=None, image_size=None, latent_shape=None):
        ...
        if self._is_stable_diffusion_3():
            ...
            if image_size is None:
                if hasattr(self.model, "backbone") and hasattr(self.model.backbone, "image_shape"):
                    ...
                    if hasattr(self.model, "input") and isinstance(self.model.input, dict):
                        if "images" in self.model.input:
                            img_shape = self.model.input["images"].shape
                            if img_shape[1] is not None and img_shape[2] is not None:
            ...
            if latent_shape is None:
                if hasattr(self.model, "backbone") and hasattr(self.model.backbone, "latent_shape"):
                    ...
                    # Default latent shape for SD3 (typically 1/8 of image size with 16 channels)
