...
@keras_nlp_export("keras_nlp.models.Segmentation")
class Segmentation(Task):
    """Base class for all segmentation tasks.

    `Segmentation` tasks wrap a `keras_nlp.models.Backbone` to create a model
    that can be used for segmentation.
    `Segmentation` tasks take an additional
    ...
    All `Segmentation` tasks include a `from_preset()` constructor which can be
    used to load a pre-trained config and weights.

    Example:
    ```python
    model = keras_nlp.models.Segmentation.from_preset(
        "basnet_resnet",
        num_classes=2,
    )
