...
class PaliGemmaCausalLM(CausalLM):
    ...
    def call_with_cache(
        ...
        Args:
            ...
            images: a dense float Tensor with shape `(batch_size, resolution, resolution, 3)`.
        ...
        # TODO: Eventually, we should cache this as well.
        ...
        for i, transformer_layer in enumerate(self.backbone.transformer_layers):
            ...
            complete_sequence, next_cache = transformer_layer(
                complete_sequence,
                cache=current_cache,
        ...
        hidden_states = x = self.backbone.layer_norm(complete_sequence)
        logits = self.backbone.token_embedding(x, reverse=True)
        return logits, hidden_states, cache

    def _build_cache(self, token_ids, images):
        """Build an empty cache for use with `call_with_cache()`."""
        batch_size = ops.shape(token_ids)[0]
        max_length = (
            ops.shape(token_ids)[1] + 0
        )  # TODO: Replace this with (res / patch_size) ^ 2
    ...
    def generate_step(
        ...
        hidden_states, cache = self._build_cache(token_ids, images)
        # Compute the lengths of all user inputted tokens ids.
        row_lengths = ops.sum(ops.cast(padding_mask, "int32"), axis=-1)
        # Start at the first index that has no user inputted id.
        index = ops.min(row_lengths)

        def next(prompt, image, cache, index):
            # The cache index is the index of our previous token.
            cache_update_index = index - 1
            batch_size = ops.shape(prompt)[0]
            prompt = ops.slice(
                prompt, [0, cache_update_index], [batch_size, 1]
            )  # TODO: understand this call more
        ...
        token_ids = self.sampler(  # TODO: Update sampler with images
            next=next,
            prompt=token_ids,
            image=images,
            cache=cache,
        ...
        return {
            ...
            "images": images,  # TODO: Confirm this is correct
        }
    ...
    def score(
        ...
        Args:
            ...
            images: A <float32>[batch_size, resolution, resolution, 3] tensor containing
