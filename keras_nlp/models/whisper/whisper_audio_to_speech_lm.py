...
#     encoder inputs and the previous tokens. You can finetune `WhisperAudioToSpeechLM` to
...
#         return WhisperAudioToSpeechLMPreprocessor
...
#     def call_encoder(self, features, padding_mask):
#         """Does a forward pass on the encoder and returns the encoder output."""

#         # Embedding layers.
#         embedded_features = self.backbone.get_layer("encoder_token_embedding_conv_layer_1")(features)
#         embedded_features = keras.activations.gelu(embedded_features, approximate=False)
#         embedded_features = tf.pad(
#             embedded_features, paddings=[[0, 0], [1, 1], [0, 0]]
#         )
#         embedded_features = self.backbone.get_layer("encoder_token_embedding_conv_layer_2")(embedded_features)
#         embedded_features = keras.activations.gelu(embedded_features, approximate=False)

#         token_embedding = self.backbone.get_layer("token_embedding")(features)
#         position_embedding = self.backbone.get_layer(
