...
def update_model_cards_on_hugging_face(presets):
    # Prefer using `kagglehub` client (available in our virtualenv) which
    # exposes a simple HTTP client for Kaggle's v1 API. Fall back to the
    ...
    if kagglehub is not None:
        ...
        def _get_model_metadata(owner, slug):
            # Try several possible method names that different kaggle clients
            # have used historically. If none exist, raise AttributeError so
            # the caller can skip updating the model card.
    ...
    # Track successes and failures so we can log/report models that
    # weren't updated successfully.
    failed_updates = []
    successful_updates = []
    ...
    # Write a simple JSON report for any failed model-card updates so
    # callers can inspect which models failed and why.
    if failed_updates:
        report_path = "tools/admin/hf_model_card_update_errors.json"
        try:
            with open(report_path, "w") as report_file:
                json.dump(failed_updates, report_file, indent=2)
            print(f"Wrote model-card update failures to {report_path}")
        except Exception as e:
