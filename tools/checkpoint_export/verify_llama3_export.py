"""Llama3 KerasHub → HuggingFace Export Verification Script.
...
Usage:
    ...
    KERAS_BACKEND=torch python3 \
        tools/checkpoint_export/verify_llama3_export.py \
        --preset llama3_8b_en

    # Skip generation (faster, logit-only check):
    KERAS_BACKEND=torch python3 \
        tools/checkpoint_export/verify_llama3_export.py \
        --preset llama3_8b_en --skip_generation

    # Custom export directory:
    KERAS_BACKEND=torch python3 \
        tools/checkpoint_export/verify_llama3_export.py \
        --preset llama3_8b_en --export_dir /tmp/llama3_export

    # Provide an explicit HF model ID (e.g. for a gated variant):
    KERAS_BACKEND=torch python3 \
        tools/checkpoint_export/verify_llama3_export.py \
        --preset llama3.1_8b \
...
from keras_hub.src.models.llama3.llama3_causal_lm import (  # noqa: E402
    Llama3CausalLM,
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify Llama3 KerasHub → HF export"
    )
    parser.add_argument(
        "--preset",
        default="llama3_8b_en",
        help=(
            f"KerasHub preset name (default: llama3_8b_en). "
