...
def download_files(preset, hf_model_name):
    ...
    # Vocab.
    spm_path = os.path.join(extract_dir, "spiece.model")
    response = requests.get(
        f"https://huggingface.co/{hf_model_name}/resolve/main/spiece.model"
    )
...
def convert_checkpoints(preset, keras_nlp_model, hf_model):
    ...
    cfg["num_heads"] = hf_cfg["num_attention_heads"]
    cfg["num_groups"] = hf_cfg["num_hidden_groups"]
    cfg["num_inner_repetitions"] = hf_cfg["inner_group_num"]
    cfg["embedding_dim"] = hf_cfg["embedding_size"]
    cfg["hidden_dim"] = hf_cfg["hidden_size"]
    cfg["intermediate_dim"] = hf_cfg["intermediate_size"]
    cfg["dropout"] = hf_cfg["hidden_dropout_prob"]
    cfg["max_sequence_length"] = hf_cfg["max_position_embeddings"]
    cfg["num_segments"] = hf_cfg["type_vocab_size"]
    print("Config:", cfg)
