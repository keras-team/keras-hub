...
def run_test(test_file: str, test_method: str = None) -> Tuple[bool, str]:
    """
...
def extract_model_name_from_test_file(test_file: str) -> str:
    ...
    # e.g., "keras_hub/src/models/gpt2/gpt2_causal_lm_test.py" -> "gpt2_causal_lm"
...
def categorize_model(model_name: str) -> str:
    ...
    if "causal_lm" in model_name or "gpt2" in model_name or "mistral" in model_name or "gemma" in model_name or "llama" in model_name or "phi3" in model_name or "qwen" in model_name:
        return "CausalLM"
    elif "text_classifier" in model_name or "bert" in model_name or "roberta" in model_name or "albert" in model_name or "deberta" in model_name or "f_net" in model_name or "roformer" in model_name or "xlm_roberta" in model_name or "distil_bert" in model_name:
        return "TextClassifier"
    elif "image_classifier" in model_name or "resnet" in model_name or "efficientnet" in model_name or "densenet" in model_name or "mobilenet" in model_name or "vgg" in model_name or "vit" in model_name or "deit" in model_name or "xception" in model_name or "mit" in model_name or "hgnetv2" in model_name or "cspnet" in model_name:
        return "ImageClassifier"
    elif "object_detector" in model_name or "retinanet" in model_name or "d_fine" in model_name:
...
def run_all_tests() -> Dict[str, Dict]:
    ...
    for test_file in INDIVIDUAL_TEST_FILES:
        ...
        if "gemma3" in model_name:
            ...
            passed1, output1 = run_test(test_file, "test_litert_export")
            passed2, output2 = run_test(test_file, "test_litert_export_multimodal")
            passed = passed1 and passed2
            output = output1 + "\n" + output2
        else:
            passed, output = run_test(test_file, "test_litert_export")
    ...
    if Path(PARAMETRIZED_TEST_FILE).exists():
        passed, output = run_test(PARAMETRIZED_TEST_FILE)
        print(f"Parametrized tests: {'PASSED' if passed else 'FAILED'}")

        # Parse parametrized test results to extract individual model results
        ...
        print(f"Warning: Parametrized test file {PARAMETRIZED_TEST_FILE} not found")
...
def update_markdown(results: Dict[str, Dict], models_without_tests: List[str]):
...
def main():
    """Main function."""
    print("Starting LiteRT export test coverage analysis...")
    ...
    # Update markdown
    update_markdown(results, models_without_tests)

    # Print summary
    total_tests = len([r for r in results.values() if r["category"] != "Parametrized"])
    passed_tests = len([r for r in results.values() if r["passed"] and r["category"] != "Parametrized"])
